...
| `lib/stream/xlsx/**` (streaming reader/writer)                                                                   | `yarn mocha --require spec/config/setup.js spec/integration/workbook-xlsx-reader.spec.js spec/integration/workbook-xlsx-writer spec/integration/worksheet-xlsx-writer.spec.js --recursive` plus `yarn test:end-to-end` (`express.spec.js` streams over HTTP on fixed port 3003) |
...
- Do NOT use `yarn test:typescript` — it needs a prior build, is not in CI, and is currently
  broken; CI's type check is the tsc command above
