...
class XLSX {
  ...
  async load(data, options) {
    ...
    const limits = new ZipLimits(options, ZipLimits.BUFFERED_DEFAULTS)
    let files
    try {
      // A Node Buffer/Uint8Array can go straight to fflate; only wrap other
      // inputs (e.g. ArrayBuffer) so large buffers aren't needlessly copied.
      const bytes =
        buffer instanceof Uint8Array ? buffer : new Uint8Array(buffer)
      files = unzipSync(bytes, {
        // Runs before each entry is inflated. fflate allocates exactly the
        // declared originalSize for a deflated entry (and copies `size` bytes
        // for a stored one), so this bounds memory up front.
        filter: (file) => {
          limits.addEntry()
          limits.addBytes(Math.max(file.size, file.originalSize))
          limits.addCompressedBytes(file.size, bytes.length)
          return true
        },
      })
    } catch (error) {
      if (error.code === ZipLimits.ERROR_CODE) {
        throw error
      }
      throw new Error(
    ...
    // Zero-copy Buffer view over a fflate entry, so the inflated bytes for each
    // entry aren't copied again while streaming/decoding large workbooks.
    const asBuffer = (u8) =>
      Buffer.from(u8.buffer, u8.byteOffset, u8.byteLength)
    for (const [rawEntryName, u8] of Object.entries(files)) {
      /* eslint-disable no-await-in-loop */
      // fflate encodes directory records as keys ending in '/'
      if (!rawEntryName.endsWith('/')) {
        ...
        if (
          ...
          if (process.browser) {
            ...
            content = bufferToString(asBuffer(u8))
          } else {
            // running in node.js
            content = asBuffer(u8).toString('utf8')
          }
