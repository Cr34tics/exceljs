...
const unzipLimited = require('../utils/unzip-limited')
...
class XLSX {
  ...
  async load(data, options) {
    ...
    let files
    try {
      // A Node Buffer/Uint8Array can be unzipped as is; only wrap other
      // inputs (e.g. ArrayBuffer) so large buffers aren't needlessly copied.
      const bytes =
        buffer instanceof Uint8Array ? buffer : new Uint8Array(buffer)
      files = unzipLimited(bytes, limits)
    } catch (error) {
    ...
    // Zero-copy Buffer view over an unzipped entry, so the inflated bytes for each
    ...
    for (const [rawEntryName, u8] of Object.entries(files)) {
      ...
      // directory records are keys ending in '/'
