module.exports = async function* iterateStream(stream) {
  // A stream that already failed, ended or was partly read by someone else
  // will never give this iterator all of its data and an 'end' or 'error'
  if (stream.errored) throw stream.errored
  if (stream.readableEnded || stream.readableDidRead) {
    throw new Error(
  ...
  try {
    ...
    // that breaks) must not leave this listener collecting whatever is later
