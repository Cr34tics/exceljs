...
function readSizes(data, extra, extraLength, zip64, sizes) {
  ...
  for (let b = extra; b + 4 < end; b += 4 + u16(data, b + 2)) {
    if (u16(data, b) === 1) {
      ...
      let field = b + 4
      const fieldEnd = Math.min(field + u16(data, b + 2), end)
      const next = () => {
        // within the field's own length, not just the data: past it lie the
...
function unzipLimited(data, limits) {
  ...
  entries.forEach((entry) => {
    ...
    if (entry.compression === 0) {
      // a copy, so a kept entry doesn't pin the whole archive in memory
      // (Buffer's own slice() would return a view)
      files[entry.name] = Uint8Array.prototype.slice.call(compressed)
