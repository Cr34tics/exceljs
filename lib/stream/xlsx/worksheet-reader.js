...
class WorksheetReader extends EventEmitter {
  ...
  // The worksheet's stream can be read only once, so later read() calls share
  // the first one (the workbook reader's own read() may be one of them)
  read() {
    if (!this._reading) {
  ...
  async *parse() {
    ...
    if (this._parsed) {
      throw new Error('Worksheet was already read: read or iterate it once')
    }
    this._parsed = true
