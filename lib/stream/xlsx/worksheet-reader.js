...
class WorksheetReader extends EventEmitter {
  ...
  async read() {
    try {
      ...
      // Report it to listeners, and always reject read(), which the workbook
      // reader awaits: a listener mustn't turn a corrupt worksheet into a
      // successful read. (Emitting with no listener would throw, so only emit
      // to listeners.)
