...
class WorksheetReader extends EventEmitter {
  ...
  // The worksheet's stream can be read only once. Later read() calls share the
  // first one, and a read() after the worksheet was iterated leaves its rows
  // to that iteration (the workbook reader's own read() may be either).
  read() {
    if (!this._reading) {
      this._reading = this._parsed ? Promise.resolve() : this._read()
    }
  ...
  async _read() {
    try {
      ...
      if (this.listenerCount('error') > 0) {
        // Reported to a listener: a caller that doesn't await read() mustn't
        // also get an unhandled rejection. Whoever awaits it still sees it.
        this._reading.catch(() => {})
      }
  ...
  async *parse() {
    // a second pass would find the stream consumed and silently yield nothing
    if (this._parsed) {
      throw new Error('Worksheet was already read: iterate it only once')
    }
    this._parsed = true
