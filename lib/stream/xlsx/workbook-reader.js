    ...
    try {
      for await (const { eventType, value } of this.parse(input, options)) {
        switch (eventType) {
          ...
          case 'hyperlinks':
            this.emit(eventType, value)
            // Like worksheets: parse them before the entry is drained, so a
            // listener that defers its read() doesn't find it consumed
            await value.read()
            break
        }
      }
      ...
      for await (const rawEntry of iterateStream(zip)) {
        ...
        switch (part && part.type) {
          ...
          case 'worksheet':
            ...
            break
          case 'hyperlinks':
            yield* this._parseHyperlinks(entry, part.sheetNo)
            break
          default:
            break
        }
