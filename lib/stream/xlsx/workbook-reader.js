...
// Parts parse() reads. The package rels are recognised but not parsed.
const PARTS = new Map([
...
class WorkbookReader extends EventEmitter {
  ...
  async *parse(input, options) {
    ...
    // A stream that already failed or was closed never emits again, so piping
    // it would leave the zip parser waiting forever
    if (stream.errored) {
      throw stream.errored
    }
    if (stream.destroyed) {
      throw new Error('Could not read input: the stream was already closed')
    }
      ...
      for await (const rawEntry of iterateStream(zip)) {
        ...
        switch (part?.type) {
          ...
          case 'worksheet':
            ...
            break
          case 'hyperlinks': {
            const hyperlinksReader = yield* this._parseHyperlinks(
              entry,
              part.sheetNo,
            )
  ...
  *_parseWorksheet(iterator, sheetNo) {
    ...
    const matchingSheet =
      matchingRel &&
      // this.model is missing when the archive has no xl/workbook.xml
      ((this.model && this.model.sheets) || []).find(
