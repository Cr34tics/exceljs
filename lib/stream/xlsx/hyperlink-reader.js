...
class HyperlinkReader extends EventEmitter {
  ...
  async _read() {
    ...
    try {
      ...
      // Report it to listeners, and always reject read(): the workbook reader
      // awaits this same promise, so the error also fails the workbook read,
      // and a caller that doesn't await its own read() gets no unhandled
      // rejection. (Emitting with no listener would throw, so only emit to
      // listeners.)
