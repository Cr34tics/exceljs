  ...
  async _read() {
    ...
    try {
      ...
      if (this.listenerCount('error') > 0) {
        ...
        // Rejects read(). The workbook reader awaits this same promise, so the
        // error also fails the workbook read, and a caller that doesn't await
        // its own read() gets no unhandled rejection.
        throw error
      }
    }
  }
