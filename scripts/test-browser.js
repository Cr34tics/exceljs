...
async function runTests() {
  ...
  try {
    ...
    try {
      const hasExcelJS = await page.evaluate(
        () => typeof window.ExcelJS === 'object' && window.ExcelJS !== null,
      )
      report(
        'bundle should define ExcelJS global',
        hasExcelJS,
        !hasExcelJS && 'ExcelJS is not defined',
      )
    } catch (err) {
    ...
    try {
      const hasWorkbook = await page.evaluate(
    ...
    try {
      const hasEnums = await page.evaluate(
        () =>
          window.ExcelJS.ValueType !== undefined &&
          window.ExcelJS.FormulaType !== undefined,
      )
      report('bundle should expose ValueType and FormulaType enums', hasEnums)
    } catch (err) {
      report(
        'bundle should expose ValueType and FormulaType enums',
        false,
        err.message,
      )
    }
    ...
    try {
      ...
      report(
        ...
        ok,
        !ok &&
          `Expected: ${JSON.stringify(expected)}, Got: ${JSON.stringify(csvContent)}`,
      )
    } catch (err) {
    ...
    try {
      const result = await page.evaluate(async () => {
        ...
        return {
          hasProtection:
            ws2.sheetProtection !== undefined && ws2.sheetProtection !== null,
          a1: ws2.getCell('A1').value,
        }
      })
      const ok = result.hasProtection && result.a1 === 'Protected content'
      report(
        'should support worksheet protection with password hashing',
        ok,
